...
10. For Cloudflare staging deploys, set `CLOUDFLARE_D1_DATABASE`, `CLOUDFLARE_D1_DATABASE_ID`, and `VITE_API_BASE_URL`, then run `npm run deploy:cloudflare:staging` after `wrangler login`.
...
- The ops dashboard now keeps the admin token in memory only, and the onboarding/staff apps keep session tokens in the active tab only.
